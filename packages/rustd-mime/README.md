...
- **`readForm` does not spill to temp files.** File parts stay in `FileHeader.content` even when Go would write a temp file. Non-file values use Go's `maxMemory + 10MiB` budget (headers + name overhead + body); over that, or over the part cap, throws `MessageTooLargeError`. File-part `maxMemory` / disk spill stay later.
...
Recorded after `napi build --platform --release` on linux-x64-gnu, Rust 1.97.1 (2026-09-14, checkpoint 25): **517,600 bytes** / 2,000,000.
