...
Go `mime`, `mime/quotedprintable`, and `mime/multipart` (Writer WriteField / CreateFormField / CreateFormFile / CreatePart, Reader NextPart / NextRawPart including 1-byte and mid-boundary `write`, Reader ReadForm part-count, non-file maxMemory, file maxMemory throw, and ReadForm 1-byte / mid-boundary feeds) for Node via napi-rs.

This slice implements media types, the extension table, RFC 2047 encoded-words, quoted-printable, `MIMEHeader` (`Record<string, string[]>` + `canonicalMIMEHeaderKey` / Get/Set/Add/Del/Values), `MultipartWriter.writeField` / `createFormField` / `createFormFile` / `createPart`, `fileContentDisposition`, and `MultipartReader.nextPart` / `nextRawPart` / `readForm`. The same complete body fed whole, one byte at a time, or split inside `--boundary` with seed `0x4d494d45` yields the same part sequence (issue #9 §4.3). `nextPart` returns `null` until a complete part is buffered. Empty boundary and a header line without a colon throw the same `MultipartError` strings as Go. A truncated body (missing closer) stays at `null` rather than Go's `unexpected EOF`. Reader `nextPart` accepts a >70-character boundary (RFC 2046 / Go `SetBoundary` cap applies only to Writer). LF-only part headers match Go; a header block without a blank CRLF is the same missing-colon error. A part with no `Content-Disposition` is still returned (`formName`/`fileName` empty), matching Go `NextPart`. `nextPart` of 1000 and 1001 parts both succeed, matching Go 1.24 (`NextPart` has no part-count cap; `GODEBUG=multipartmaxparts` applies to `ReadForm` only). `readForm` of 1000 parts succeeds vs Go; 1001 is `MessageTooLargeError` (`multipart: message too large`). `maxParts` matches `GODEBUG=multipartmaxparts`. `readForm` applies Go `maxMemory + 10MB` to non-file values (header snapshot + name overhead + body) and Go `maxMemory` to in-memory file content; a file that would spill to disk in Go throws `MessageTooLargeError` instead. `nextPart` matches Go's quoted-printable CTE auto-decode. `rustd-net` is out of scope; this is the single `Part.header` type (issue #9).
...
Checkpoint 25: `readForm` non-file `maxMemory` vs Go 1.24 (issue #9 §4.4). Budget is `maxMemory + 10MiB`. Each value part's header parse uses the remaining snapshot (400 + 200 per new key + value bytes); then the name, 200-byte map overhead, and body are deducted. Just-under / exact-at the Go threshold match `MessageTooLargeError` vs success. File parts stay in `FileHeader.content` (no disk spill).

Checkpoint 26: `readForm` file `maxMemory` vs Go 1.24 (issue #9 decision 3). In-memory file content uses Go's `maxMemory` budget (not the +10MiB reserve). A file that fits stays in `FileHeader.content` and deducts from both budgets. A file that would make Go write a temp file throws `MessageTooLargeError` (`multipart: message too large`). Empty files fit `maxMemory=0`. Still no temp files.

Checkpoint 27: `readForm` of the same complete body, fed whole, one byte at a time, or split inside `--boundary` with seed `0x4d494d45`, matches Go `ReadForm` Value/File for mixed value+file forms, fake `--b` substrings in values/files, and missing `Content-Disposition` (empty `FormName` skipped). Malformed missing-colon / header-without-blank-CRLF / empty boundary throw the same `MultipartError` strings as Go on all three feeds. A truncated body (no closer) stays an empty form here; Go `ReadForm` is `unexpected EOF`. Still no temp files.

## Differences from Go

- **Incremental `write` is non-blocking.** Go `NextPart` reads from an `io.Reader` until a part or EOF. Here `nextPart` / `nextRawPart` return `null` when the current buffer does not yet hold a complete part (or the closing delimiter). A truncated body that never gets the rest of the bytes stays at `null` rather than `multipart: NextPart: unexpected EOF`. `readForm` is one-shot after `write`: whole-body, 1-byte, and seeded mid-boundary splits of the same complete body produce the same form. A truncated body stays an empty form rather than Go `ReadForm`'s `unexpected EOF`. `Part.header` is this package's `MIMEHeader` (`Record<string, string[]>` with canonical keys). `net/textproto` itself is not exported. `Part.close()` then `read()` throws (`multipart: part is closed`); Go `Part.Close` then `Read` returns EOF. `createPart` writes header keys as provided (Go `CreatePart`); `mimeHeaderSet` / `Add` canonicalize first.
...
- **`readForm` does not spill to temp files.** File content that fits in `maxMemory` stays in `FileHeader.content`. File content over `maxMemory` (the Go disk-spill threshold, including the shared remaining budget across files) throws `MessageTooLargeError` instead of writing a temp file. Go `ReadForm` still returns the file via `FileHeader.Open()`. Non-file values use Go's `maxMemory + 10MiB` budget (headers + name overhead + body); over that, or over the part cap, throws `MessageTooLargeError`.
...
Recorded after `napi build --platform --release` on linux-x64-gnu, Rust 1.97.1 (2026-09-15, checkpoint 27): **517,680 bytes** / 2,000,000.
