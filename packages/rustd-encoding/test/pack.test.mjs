...
test('npm pack installs CJS and ESM with the native binary', () => {
  ...
  const temp = mkdtempSync(join(tmpdir(), 'rustd-encoding-pack-'));
  try {
    ...
    const code = `
      const a = require('rustd-encoding');
      import('rustd-encoding').then((b) => {
        for (const k of Object.keys(a)) if (a[k] !== b[k]) throw new Error(k + ' differs');
        const src = new Uint8Array([0xab, 0xcd]);
        if (a.hexEncode(src) !== 'abcd') throw new Error('native hexEncode failed');
        const b64 = a.Base64Encoding.Std.encodeToString(src);
        if (b64 !== 'q80=') throw new Error('native base64 failed: ' + b64);
        console.log('rustd-encoding packed CJS + ESM OK');
      });
    `;
    const result = spawnSync(process.execPath, ['-e', code], { cwd: clean, encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr + result.stdout);
    assert.match(result.stdout, /rustd-encoding packed CJS \+ ESM OK/);
  } finally {
