...
test('npm pack installs CJS and ESM with the native binary', () => {
  ...
  const temp = mkdtempSync(join(tmpdir(), 'rustd-serial-pack-'));
  try {
    ...
    const code = `
      const a = require('rustd-serial');
      import('rustd-serial').then((b) => {
        for (const k of Object.keys(a)) if (a[k] !== b[k]) throw new Error(k + ' differs');
        const rows = new a.CsvReader(Buffer.from('a,b\\n1,2\\n')).readAll();
        if (JSON.stringify(rows) !== JSON.stringify([['a','b'],['1','2']])) {
          throw new Error('native csv failed: ' + JSON.stringify(rows));
        }
        const pem = a.pemEncode({ type: 'FOO', bytes: Buffer.from('hello') });
        const found = a.pemDecode(pem);
        if (!found || found.block.type !== 'FOO' || Buffer.from(found.block.bytes).toString() !== 'hello') {
          throw new Error('native pem failed');
        }
        const schema = {
          name: 'person',
          kind: 'element',
        ...
        const xml = a.xmlMarshal({ id: 1, name: 'Ann' }, schema);
        const again = a.xmlUnmarshal(xml, schema);
        if (again.id !== 1 || again.name !== 'Ann') throw new Error('native xml failed');
        console.log('rustd-serial packed CJS + ESM OK');
      });
    `;
    const result = spawnSync(process.execPath, ['-e', code], { cwd: clean, encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr + result.stdout);
    assert.match(result.stdout, /rustd-serial packed CJS \+ ESM OK/);
  } finally {
