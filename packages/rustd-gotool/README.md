...
Checkpoint 22 of [issue #28](https://github.com/AkaraChen/rustd-js/issues/28):
...
`makeComplex(-re, -im)`), and Complex `constCompareOp` EQL/NEQ vs Go
`MakeBool(Compare)` (component EQL after `match`/`vtoc`; Unknown vs Complex
can be NEQ true).
`go/format` / `gofmt`, String Compare, and
...
- `GoConstValue` is an opaque handle (Go's `constant.Value` is an interface).
  ...
  LSS/LEQ/GTR/GEQ and Bool/String operands throw (Go panics on Complex
  ordering; String Compare stays later). `constSign` throws on Bool/String
...
Local Linux x64 GNU release probe, Rust 1.97.1 (2026-09-15): **787,136 bytes**.

```text
$ ls -l packages/rustd-gotool/*.node
-rwxrwxr-x 1 akrc akrc 787136 Sep 15 07:44 packages/rustd-gotool/rustd-gotool.linux-x64-gnu.node
```
