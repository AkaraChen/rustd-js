...
- `iotest` readers (`dataErrReader`, `errReader`, `halfReader`, `oneByteReader`,
  ...
  Go 1.24 `(n, err)` sequences live in `test/iotest-fixtures.json`, generated by
  `mise exec -- go run ./tools/gofixtures/iotestcheck`. HalfReader uses Go's
  `(len(p)+1)/2`, not the issue-draft `len(p)/2`. DataErrReader merges a later
  error onto already-buffered data; an inner `(data, EOF)` on one call still
  surfaces as `n=0, EOF` (recorded, not reinterpreted).
