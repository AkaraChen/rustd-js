...
test('host platform .node is stripped and under 2MB (expected < 500KB after Zipf/ziggurat)', () => {
  ...
  assert.ok(bytes <= SIZE_CAP, `${binary}: ${bytes} bytes exceeds 2MB`);
  assert.ok(bytes < SIZE_EXPECT, `${binary}: ${bytes} bytes suggests unexpected deps`);
...
test('npm pack installs CJS and ESM with the native binary', () => {
  ...
  const temp = mkdtempSync(join(tmpdir(), 'rustd-mathx-pack-'));
  try {
    ...
    const code = `
      const a = require('rustd-mathx');
      import('rustd-mathx').then((b) => {
        for (const k of Object.keys(a)) if (a[k] !== b[k]) throw new Error(k + ' differs');
        if (a.leadingZeros64(1n) !== 63) throw new Error('leadingZeros64');
        const r = a.newPCG(1n, 2n);
        if (r.uint64() !== 14192431797130687760n) throw new Error('pcg');
        const z = new a.Zipf(a.newPCG(1n, 2n), 1.1, 1, 100).uint64();
        if (typeof z !== 'bigint') throw new Error('zipf');
        console.log('rustd-mathx packed CJS + ESM OK');
      });
    `;
    const result = spawnSync(process.execPath, ['-e', code], { cwd: clean, encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr + result.stdout);
    assert.match(result.stdout, /rustd-mathx packed CJS \+ ESM OK/);
  } finally {
