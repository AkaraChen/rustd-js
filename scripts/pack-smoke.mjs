...
for (const { dir, manifest } of packages()) {
  ...
  try {
    ...
    cpSync(join(dir, 'npm', platform), nativeDir, { recursive: true });
    cpSync(join(dir,binary),join(nativeDir,binary));
    const nativeManifest = JSON.parse(readFileSync(join(nativeDir, 'package.json'), 'utf8'));
    assert.equal(nativeManifest.name, `${manifest.name}-${platform}`);
    assert.equal(nativeManifest.version, manifest.version);
    assert.equal(nativeManifest.main, binary);
    ...
    const installed = JSON.parse(readFileSync(join(clean, 'node_modules', manifest.name, 'package.json'), 'utf8'));
    assert.deepEqual(installed.optionalDependencies, manifest.optionalDependencies);
    for (const version of Object.values(installed.optionalDependencies)) assert.equal(version, manifest.version);
